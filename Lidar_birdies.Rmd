...
# load in dsm created in LASTools
dsm1 <- 
   raster("data/final_duke_dsm.tif")
dsm <- 
   crop(dsm1, extent(duke)+c(-250,250,-250,250))

fix = read.csv("data/pixels_to_fix.csv") %>%
   mutate(x = E + 0.5, 
          y = formatC(N + 0.5, digits = 1, format = "f"),
          keyx = as.character(x),
          keyy = as.character(y))
...
# Make a raster with the corrected elevation values
```{r}
# get values
vals <- 
   dp$value.final2
